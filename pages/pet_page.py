...
class PostPetsPage(BaseListingPage):

    def __init__(self, page: Page):
        super().__init__(page)

        self.upload_input = page.locator("#sec-photos input[type='file']")
        self.breed = page.locator("input[type='text']").first
        self.age_months = page.locator("input[inputmode='numeric'][min='0'][max='240']")
    ...
    def select_category(self):
        btn = self.page.locator("button[title='Pets & Animals']")
    ...
    def set_papers(self, papers: bool = True):
        toggle = self.page.locator("button[aria-pressed]:has-text('Papers')")
        toggle.wait_for(state="visible", timeout=8000)
        is_pressed = toggle.get_attribute("aria-pressed")
        if papers and is_pressed == "false":
            toggle.click()
        elif not papers and is_pressed == "true":
            toggle.click()